...
public class EmailService {
    ...
    @Async
    public void sendRegMail(Part part) throws MessagingException {

        final String recipientName = part.getName() + " " + part.getSurname();
        final String recipientEmail = part.getEmail();
        ...
        part.getNominations().forEach( a -> { if (a.isSelected()){
            nominations.add(a.getName());
        }});

        ctx.setVariable("hobbies", nominations);

        // Prepare message using a Spring helper
        final MimeMessage mimeMessage = this.mailSender.createMimeMessage();
        final MimeMessageHelper message = new MimeMessageHelper(mimeMessage, "UTF-8");
        message.setSubject("Регистрация в XXI Чемпионате.");
        message.setFrom("director@alliancebeauty.ru");
        message.setTo(recipientEmail);
